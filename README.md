...
Two further workflows are a **manual-only** scaffold for the *future* next control plane ("mamai backend new") on R2 artifact storage. Neither has a `push` trigger, neither performs a cutover, and neither modifies `supabase.yml`, `autopilot-execution.yml`, or the current live Supabase project.

- `supabase-next.yml` ("Deploy Supabase Next") migrates the next control-plane schema and deploys the Edge Functions to the next project. It runs only on `workflow_dispatch` with `confirm` equal to exactly `DEPLOY_NEXT_SUPABASE`, and -- until this migration branch merges -- only from `autopilot/migrate-next-supabase-r2`. It then preflights every required secret by name, derives the 20-character project ref from `AUTOPILOT_NEXT_SUPABASE_URL` (validated, never printed), runs `pnpm db:migrate` against `AUTOPILOT_NEXT_DATABASE_URL`, and sets the next project's Edge runtime variables.
- `autopilot-execution-next.yml` is the isolated twin of the execution workflow, with the same job-id-only input, per-job concurrency and timeout, bound to `AUTOPILOT_NEXT_DATABASE_URL` and the `AUTOPILOT_NEXT_R2_*` set. It needs no Supabase Storage credential: a complete R2 configuration makes `createArtifactBlobStore` select R2 outright.
- `supabase-next-state.yml` moves the durable control-plane state between the two PostgreSQL databases in three separately-confirmed modes -- `inventory` (read-only), `copy` (one-shot), `verify` (read-only comparison) -- driven by `scripts/migrate-control-plane-state-next.ts`. **None of these workflows has been run yet.**

State migration reads the existing `AUTOPILOT_CONTROL_DATABASE_URL` as source and writes `AUTOPILOT_NEXT_DATABASE_URL` as target, and touches PostgreSQL only -- no Management API, Edge Function, R2 or object storage on either side. The source is opened with `BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY`, so PostgreSQL itself rejects any write to it. A `copy` is refused unless the source is quiet (no `QUEUED`/`DISPATCHING`/`DISPATCHED`/`CLAIMED`/`RUNNING` job and no task in `IMPLEMENTING`/`TESTING`/`REVIEWING`) and the target's operational tables are still empty after `pnpm db:migrate`; it runs in one target transaction under an advisory lock, so any failure rolls the target back untouched.

Two things are deliberately left behind. `autopilot_operators` and `autopilot_project_memberships` are bound to the current project's Supabase Auth user ids, which do not exist in the next project's Auth namespace; `createEdgeRuntime` re-creates the operator row from the existing email/project allowlists on first authenticated request instead. And externalized artifact blobs stay in the current Supabase Storage: rows keep their persisted `storage.provider`, so `RoutingArtifactBlobStore` reads pre-cutover artifacts through the legacy reader while new ones are written to R2.

`SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are never passed through `supabase secrets set` -- Supabase reserves the `SUPABASE_` prefix and injects both into every hosted Edge Function automatically, so the deployed runtime reads the next project's own pair. Historical `provider === "supabase"` artifacts stay readable because the deploy hands the *existing* `AUTOPILOT_SUPABASE_URL` / `AUTOPILOT_SUPABASE_SERVICE_ROLE_KEY` to the new runtime as its `AUTOPILOT_LEGACY_SUPABASE_*` read credentials; no old secret is duplicated or changed. See `SECRETS_MANIFEST.md`.
