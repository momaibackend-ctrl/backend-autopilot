...
export class GitHubActionsDispatcher implements ExecutionJobDispatcher {
  constructor(private readonly token:string,private readonly controlRepository:string,private readonly workflow='autopilot-execution.yml',private readonly ref='autopilot/v0.4-remote-runtime'){
  ...
  async dispatch(job:ExecutionJob){
    const response=await fetch(`https://api.github.com/repos/${this.controlRepository}/actions/workflows/${encodeURIComponent(this.workflow)}/dispatches`,{method:'POST',headers:{accept:'application/vnd.github+json',authorization:`Bearer ${this.token}`,'content-type':'application/json','user-agent':'backend-autopilot/0.4','x-github-api-version':'2022-11-28'},body:JSON.stringify({ref:this.ref,inputs:{job_id:job.id}})});
