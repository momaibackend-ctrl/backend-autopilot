...
import { GitHubRestRepositoryProvider } from '../../adapters/github/src/repository-provider.js';
...
import { SuperadminService } from '../../superadmin/src/index.js';
...
function createConfiguredDispatcher(){const token=process.env['AUTOPILOT_GITHUB_DISPATCH_TOKEN'];const repository=process.env['AUTOPILOT_CONTROL_REPOSITORY'];return token&&repository?new GitHubActionsDispatcher(token,repository,process.env['AUTOPILOT_EXECUTION_WORKFLOW']??'autopilot-execution.yml',process.env['AUTOPILOT_CONTROL_REF']??'main'):undefined;}
