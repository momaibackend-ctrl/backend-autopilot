...
export class ArtifactStore {
  ...
  async write(projectId:string,kind:ArtifactKind,content:unknown,taskId?:string,runId?:string):Promise<Artifact>{
    ...
    const storage=this.blobs&&Buffer.byteLength(serialized,'utf8')>=this.externalizeAtBytes?await this.blobs.put({projectId,artifactId:id,body:serialized,contentType:'application/json'}):undefined;
