...
export class CommandRunner {
  ...
  async run(input:{command:string;args:string[];cwd:string;taskId:string;allowed:CommandCategory[];env?:Record<string,string>}):Promise<CommandResult>{
    ...
    const result=await new Promise<{exitCode:number;stdout:string;stderr:string}>((resolve,reject)=>{
      const child=spawn(input.command,input.args,{cwd:input.cwd,shell:false,windowsHide:true,env:{...process.env,...input.env}});let stdout='',stderr='';
    ...
    const entry={record:{command:[input.command,...input.args],cwd:input.cwd,startedAt,finishedAt:this.clock.now(),exitCode:result.exitCode,taskId:input.taskId,category},stdout:result.stdout,stderr:result.stderr};this.journal.set(input.taskId,[...(this.journal.get(input.taskId)??[]),entry]);return entry;
  }
