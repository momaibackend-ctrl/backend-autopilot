...
export type PolicyAction='PROJECT_WRITE'|'RESOURCE_READ'|'RESOURCE_WRITE'|'EXECUTE'|'MIGRATE'|'NETWORK'|'ARTIFACT_READ';
...
export class PolicyEngine {
  ...
  async authorize(request:AuthorizationRequest):Promise<void>{
    if(request.project.autonomyMode==='AUTONOMOUS_PRODUCTION') throw new UnsupportedOperation('AUTONOMOUS_PRODUCTION is technically disabled in v0.1');
    if(request.action==='EXECUTE'&&request.project.autonomyMode==='OBSERVE') throw new PolicyViolation('OBSERVE mode forbids execution');
    if((request.action==='MIGRATE'||request.action==='NETWORK')&&request.project.autonomyMode==='GUARDED') throw new PolicyViolation(`${request.project.autonomyMode} mode forbids ${request.action}`);
    if(request.project.environment==='PRODUCTION'&&request.action!=='RESOURCE_READ'&&request.action!=='ARTIFACT_READ') throw new UnsupportedOperation('Production mutation is not supported in v0.1');
    ...
    } else if(['RESOURCE_READ','RESOURCE_WRITE','MIGRATE','NETWORK'].includes(request.action)) {
