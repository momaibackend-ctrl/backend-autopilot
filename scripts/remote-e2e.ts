...
const fixtureChanges=await Promise.all(fixturePaths.map(async path=>({path,content:await githubFile(path,fixtureRef)})));
const originalRegression=fixtureChanges.find(value=>value.path==='tests/regression.test.js')!.content;
...
const first=await call<{job:{id:string}}>('task_execute',{projectId,taskId:task.id,resourceId,operationId:failingOperation,changes:[...fixtureChanges.filter(value=>value.path!=='tests/regression.test.js'),{path:'tests/regression.test.js',content:"import test from 'node:test';\nimport assert from 'node:assert/strict';\ntest('intentional remote repair probe',()=>assert.equal(1,2));\n"},{path:'src/remote-runtime-proof.js',content:"export const executionRuntime = 'github-actions';\n"}]});
