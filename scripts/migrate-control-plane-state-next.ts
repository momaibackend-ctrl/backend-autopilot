...
async function inventory() {
  ...
  await target.query(readOnlySnapshotStatement);
  try {
    return { mode, source: await describe(source, true), target: await describe(target, false), authBoundNote: `${authBoundTables.join(', ')} are ${authBoundStatus}: their user ids belong to the current project's Supabase Auth namespace` };
  } finally {
...
async function copy() {
  ...
  await sourceQuery(readOnlySnapshotStatement);
  const activity = evaluateSourceActivity({ activeExecutionJobs: await activeJobTally(source), transientTasks: await transientTaskTally(source) });
  if (activity.blocked) {
    await sourceQuery('ROLLBACK');
    throw new MigrationBlocked('Source control plane still has work in flight; copy is refused until it is quiet', {
      ...
      activeExecutionJobIds: await blockingJobIds(),
      transientTasks: activity.transientTasks,
      transientTaskIds: await blockingTaskIds(),
    });
  }

  await target.query('BEGIN');
  try {
    const locked = await target.query<{ locked: boolean }>('SELECT pg_try_advisory_xact_lock($1::bigint) AS locked', [targetAdvisoryLockKey]);
    if (!locked.rows[0]?.locked) throw new MigrationBlocked('Another control-plane state migration holds the target advisory lock');
    const readiness = evaluateTargetReadiness(await tallies(target, tablesRequiringEmptyTarget()));
  ...
  try {
    ...
    for (const entry of controlPlaneMigrationPlan) {
      if (entry.strategy === 'INSERT') {
        const left = await hashTable(source, entry);
        const right = await hashTable(target, entry);
      ...
      const left = await keyedData(source, entry);
      const right = await keyedData(target, entry);
    ...
    const authBound = [];
    for (const table of authBoundTables) authBound.push({ table, status: authBoundStatus, sourceRows: await count(source, table), targetRows: await count(target, table) });
...
async function describe(client: Client, isSource: boolean) {
  const planned = controlPlaneMigrationPlan.map(entry => entry.table);
  const present = await presentTables(client, [...planned, ...authBoundTables]);
  const tables = planned.map(table => ({ table, present: present.has(table), rows: null as number | null }));
  for (const entry of tables) if (entry.present) entry.rows = await count(client, entry.table);
  const artifactsByProvider = present.has('artifacts') ? await providerTally(client) : [];
  return {
    tables,
    activeExecutionJobs: present.has('execution_jobs') ? await activeJobTally(client) : [],
    transientTasks: present.has('tasks') ? await transientTaskTally(client) : [],
    artifactsByStorageProvider: artifactsByProvider,
    authBound: await Promise.resolve(authBoundTables.map(table => ({ table, present: present.has(table), status: 'AUTH_BOUND_NOT_PORTABLE' }))),
    role: isSource ? 'SOURCE' : 'TARGET',
  };
}

async function presentTables(client: Client, tables: readonly string[]) {
...
const count = async (client: Client, table: string): Promise<number> => Number((await client.query<{ count: string }>(countSql(table))).rows[0]?.count ?? 0);

async function tallies(client: Client, tables: readonly string[]): Promise<Tally[]> {
  const result: Tally[] = [];
  for (const table of tables) result.push({ key: table, count: await count(client, table) });
  return result;
}

async function activeJobTally(client: Client): Promise<Tally[]> {
  const result = await client.query<{ status: string; count: string }>('SELECT status, count(*)::bigint AS count FROM execution_jobs WHERE status = ANY($1::text[]) GROUP BY status ORDER BY status', [[...activeExecutionJobStatuses]]);
  return result.rows.map(row => ({ key: row.status, count: Number(row.count) }));
}

async function transientTaskTally(client: Client): Promise<Tally[]> {
  const result = await client.query<{ state: string; count: string }>("SELECT data->>'state' AS state, count(*)::bigint AS count FROM tasks WHERE data->>'state' = ANY($1::text[]) GROUP BY 1 ORDER BY 1", [[...transientTaskStates]]);
  return result.rows.map(row => ({ key: row.state, count: Number(row.count) }));
}

async function providerTally(client: Client): Promise<Tally[]> {
  const result = await client.query<{ provider: string; count: string }>("SELECT data->'storage'->>'provider' AS provider, count(*)::bigint AS count FROM artifacts WHERE data->'storage'->>'provider' IS NOT NULL GROUP BY 1 ORDER BY 1");
  return result.rows.map(row => ({ key: row.provider, count: Number(row.count) }));
}
...
const blockingJobIds = async (): Promise<string[]> =>
  (await sourceQuery<{ id: string }>('SELECT id FROM execution_jobs WHERE status = ANY($1::text[]) ORDER BY id LIMIT 20', [[...activeExecutionJobStatuses]])).rows.map(row => row.id);
const blockingTaskIds = async (): Promise<string[]> =>
  (await sourceQuery<{ id: string }>("SELECT id FROM tasks WHERE data->>'state' = ANY($1::text[]) ORDER BY id LIMIT 20", [[...transientTaskStates]])).rows.map(row => row.id);

async function hashTable(client: Client, entry: TablePlan) {
  const hasher = new RowHasher();
  for (let offset = 0; ; offset += PAGE) {
    const page = await client.query({ text: selectPageSql(entry), values: [PAGE, offset], rowMode: 'array' });
    for (const row of page.rows as unknown[][]) hasher.add(row);
...
async function keyedData(client: Client, entry: TablePlan) {
  const keyIndex = entry.columns.indexOf(entry.primaryKey[0] as string);
  const dataIndex = entry.columns.indexOf('data');
  const values = new Map<string, unknown>();
  for (let offset = 0; ; offset += PAGE) {
    const page = await client.query({ text: selectPageSql(entry), values: [PAGE, offset], rowMode: 'array' });
    for (const row of page.rows as unknown[][]) values.set(String(row[keyIndex]), row[dataIndex]);
    if (page.rows.length < PAGE) break;
  }
  return values;
}
...
async function copyTable(entry: TablePlan): Promise<number> {
  ...
  let copied = 0;
  for (let offset = 0; ; offset += PAGE) {
    const page = await source.query({ text: selectPageSql(entry), values: [PAGE, offset], rowMode: 'array' });
...
/**
 * The only door to the source connection, and it opens for reads alone. The READ ONLY transaction
 * already makes a write impossible server-side; this is the second, independent lock on the same
 * door, so a future edit cannot quietly introduce one either.
 */
async function sourceQuery<T extends Record<string, unknown>>(text: string, values: unknown[] = []) {
