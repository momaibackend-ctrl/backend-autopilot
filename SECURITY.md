...
- API Request Runner and the executable scenario runner (`superadmin_scenario_run`) share one implementation and require a project-owned active `HTTP_API` resource, `AUTONOMOUS_STAGING`, `READ`, and a non-production project. HTTPS is required; plain HTTP only for loopback hosts of a `LOCAL`/`SANDBOX` resource. Private, link-local, unique-local, CGNAT and cloud metadata targets are rejected, as are protocol-relative paths, origin escape and base-path escape. The Request Runner follows no redirects; scenario steps follow at most three, same-origin only.
- Scenario execution is bounded: 15 s per request, 3 redirects, 256 KB read from a response body, 8 KB persisted as evidence, 120 s per scenario, 20 steps. Transport errors are sanitized to a typed code and redacted message.
...
- Remote deployment exposes a static GitHub Pages console and Supabase Edge endpoints over HTTPS. Supabase Auth JWTs plus server-side operator/project membership protect Control API routes. Remote MCP and reconciliation use distinct high-entropy bearer credentials stored only as Edge/client secrets. Fastify remains local-development only.
