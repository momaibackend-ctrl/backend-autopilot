...
Deno.serve(async request=>{
  ...
  server.registerTool('superadmin_task_rebase_onto_current_base',{description:'Transfer an already-verified READY task onto the registered repository's current base branch after a dependency merge made its pull request conflict: replays the task's own commit range with a real 3-way cherry-pick, reports genuine semantic conflicts with base/current/task evidence, accepts resolutions for exactly those paths, re-runs the whole verification pipeline, then opens a fresh pull request and supersedes the stale one',inputSchema:{operationId,projectId,taskId:entityId,resourceId:entityId,resolutions:z.array(z.object({path:z.string().min(1).max(400),content:z.string().max(400000)})).max(50).default([])},annotations:{...mut,openWorldHint:true}},safe(async value=>admin().taskRebaseOntoCurrentBase(principal,value)));
