...
Deno.serve(async request=>{
  ...
  server.registerTool('superadmin_epic_run',{description:'Dispatch the epic verification workflow for one named commit. Checks out exactly that SHA (detached, verified by rev-parse), brings up real disposable PostgreSQL/Redis/object-storage, runs the target suite once, attributes results to the dimensions the members declared, records TRUSTED_CI evidence bound to that commit, and finally calls superadmin_epic_verify to produce the EPIC_VERIFICATION_REPORT. serviceEnv maps the target project own integration variable names onto the disposable services, so no project-specific names live in the control plane.',inputSchema:{operationId,projectId,epicKey:z.string().min(1),headSha:z.string().regex(/^[0-9a-f]{40}$/),resourceId:entityId,externalKeyPrefix:z.string().min(1).optional(),taskIds:z.array(entityId).optional(),serviceEnv:z.record(z.enum(['POSTGRES_URL','REDIS_URL','S3_ENDPOINT','S3_ACCESS_KEY','S3_SECRET_KEY','S3_BUCKET'])).optional()},annotations:{...mut,openWorldHint:true}},safe(async value=>admin().executeMutation(principal,'epic_run',value.projectId,value.operationId,{epicKey:value.epicKey,headSha:value.headSha},async()=>{
    ...
    return runtime.dispatcher.dispatchWorkflow(Deno.env.get('AUTOPILOT_EPIC_WORKFLOW')??'autopilot-epic-verification.yml',{epic_key:value.epicKey,head_sha:value.headSha,epic_input:JSON.stringify(payload)});
  })));
  ...
  server.registerTool('superadmin_task_execute',{description:'Queue a fixed GitHub Actions execution from semantic file changes. resourceId is optional: a project with an ACTIVE canonical development repository resolves its own target server-side, and a supplied resourceId may then only confirm it. A task that already executed always keeps the repository it was pinned to.',inputSchema:{operationId,projectId,taskId:entityId,resourceId:entityId.optional(),changes:z.array(fileChangeSchema).min(1)},annotations:{...mut,openWorldHint:true}},safe(async value=>admin().executeMutation(principal,'task_execute',value.projectId,value.operationId,{...value,changes:value.changes.map(change=>({path:change.path,operation:change.operation}))},()=>runtime.asyncExecution.enqueueImplementation(value,value.resourceId,principal.actor))));
  ...
  server.registerTool('superadmin_sandbox_pull_request_merge',{description:'Merge the pull request of a READY task into the default branch of the registered repository. Repository, PR, head and base are resolved entirely server-side and SHA-pinned to the verified FINAL_CHANGE_MANIFEST commit; no caller-supplied ref is ever accepted. PRECONDITIONS, all enforced here and none bypassable: the task is READY, which already required REQUIREMENTS_SNAPSHOT, IMPLEMENTATION_PLAN, ARCHITECTURE_REVIEW, CODE_DIFF, TEST_REPORT, SECURITY_REPORT and REVIEW_REPORT, plus a CI_REPORT produced by the real CI run for THIS exact commit; the repository is an ACTIVE registered non-production GitHub repository carrying WRITE and ADMIN; and the latest run commit equals the manifest commit on an autopilot/ branch. NO SEPARATE HUMAN CONFIRMATION IS REQUIRED OR ACCEPTED: this tool takes no confirmation token, unlike the destructive tools that do. When the preconditions hold, merging is the expected next step; do not withhold it pending an approval nobody asked you to obtain.',inputSchema:{operationId,projectId,taskId:entityId,resourceId:entityId},annotations:{...mut,openWorldHint:true}},safe(async value=>admin().executeMutation(principal,'sandbox_pull_request_merge',value.projectId,value.operationId,value,()=>mergeSandboxPullRequest(runtime,value))));
  ...
  //
  // Promotion and export are different operations and stay published under different
  // names. Promotion assigns a REGISTERED repository the role of the project's single
  // source of further development: it copies no Git, creates no repository, renames
  // nothing and moves no organization. Export transfers history to another repository and
  // on its own changes nothing about which repository is canonical. Both have a read-only
  // dry run, and both mutations pin the exact state that dry run saw.
  // ------------------------------------------------------------------------
  server.registerTool('superadmin_canonical_repository_get',{description:'Read the project ACTIVE canonical development repository and the full append-only history of which repository was canonical when. A project with no binding is a normal state, not an error.',inputSchema:{projectId},annotations:ro},safe(async({projectId})=>admin().canonicalRepositoryGet(principal,projectId)));
  server.registerTool('superadmin_canonical_repository_plan',{description:'Read-only dry run for canonical promotion. Verifies the project, that the resource is an ACTIVE project-owned GITHUB_REPOSITORY carrying the needed permissions, that the repository is reachable, that its identity matches the registration, that the default branch and its exact head commit exist and are reachable, that no conflicting promotion or unfinished repository transfer is in flight, and reports the known verification state without fabricating one. Returns the exact changes a promotion would make, every blocker with a remediation, and the expectedHeadSha and expectedCurrentCanonicalVersion the mutation must pin. Has no side effects of any kind.',inputSchema:{projectId,resourceId:entityId},annotations:ro},safe(async({projectId,resourceId})=>admin().canonicalRepositoryPlan(principal,projectId,resourceId)));
  server.registerTool('superadmin_canonical_repository_promote',{description:'Assign an already-registered repository the role of canonical development repository for one project. Re-checks every precondition against freshly read state, refuses a stale plan with STALE_PROMOTION_PLAN when the head or the current binding moved, writes the new binding atomically under a durable one-ACTIVE-per-project database invariant, supersedes the previous binding without deleting it, reads the result back before reporting success, and emits a CANONICAL_REPOSITORY_REPORT. It copies no Git history, creates no repository, renames nothing, changes no organization and moves no secret.',inputSchema:{operationId,projectId,resourceId:entityId,expectedHeadSha:z.string().regex(/^[0-9a-f]{40}$/),expectedCurrentCanonicalVersion:z.number().int().nonnegative(),confirmation:z.literal('PROMOTE_CANONICAL_DEVELOPMENT_REPOSITORY'),reason:z.string().min(8).max(500)},annotations:mut},safe(async value=>admin().canonicalRepositoryPromote(principal,value)));
  server.registerTool('superadmin_canonical_repository_rollback',{description:'Restore the previous canonical binding from history. Metadata only: it never deletes a repository, force-pushes, rewrites Git history, removes branches or tags, or touches an export target. Requires explicit confirmation, pins the binding version being undone, and verifies the repository being restored still exists, is ACTIVE and is reachable.',inputSchema:{operationId,projectId,expectedCurrentCanonicalVersion:z.number().int().positive(),confirmation:z.literal('ROLLBACK_CANONICAL_DEVELOPMENT_REPOSITORY'),reason:z.string().min(8).max(500)},annotations:destructive},safe(async value=>admin().canonicalRepositoryRollback(principal,value)));

  // Renaming a registered repository is the ONE way a GITHUB_REPOSITORY binding may move, and
  // it is allowed only because it can be proved to be a rename rather than a re-pointing.
  server.registerTool('superadmin_repository_rename_plan',{description:'Read-only dry run for renaming the repository a project is registered against. Reports the provider stable repository id, default branch and exact head commit that a rename must preserve, whether the target name is already taken, every blocker with a remediation, and the exact changes -- including that the registered resource is re-pointed in place rather than duplicated. Has no side effects.',inputSchema:{projectId,resourceId:entityId,newName:z.string().regex(/^[A-Za-z0-9_.-]{1,100}$/)},annotations:ro},safe(async({projectId,resourceId,newName})=>admin().repositoryRenamePlan(principal,projectId,resourceId,newName)));
  server.registerTool('superadmin_repository_rename',{description:'Rename a registered GitHub repository in place and re-point its registration to follow it. This is the single exception to the rule that GitHub repository bindings cannot be changed, and it earns it by proof: the provider stable repository id, the default branch and the exact head commit must all be identical before and after, and the new full name must be exactly the planned one. If any of that fails the registration is deliberately left unchanged and the mismatch is reported for a human. Keeps full Git history, branches, tags, pull requests and issues; rewrites no history, moves no secret, and cannot transfer the repository to another owner because no owner is accepted. Refused while the repository is the ACTIVE canonical development repository, so the supported order is rename first, promote second.',inputSchema:{operationId,projectId,resourceId:entityId,newName:z.string().regex(/^[A-Za-z0-9_.-]{1,100}$/),expectedCurrentReference:z.string().min(1),expectedHeadSha:z.string().regex(/^[0-9a-f]{40}$/),confirmation:z.literal('RENAME_REGISTERED_REPOSITORY'),reason:z.string().min(8).max(500)},annotations:{...destructive,openWorldHint:true}},safe(async value=>admin().repositoryRename(principal,value)));

  server.registerTool('superadmin_repository_export_plan',{description:'Read-only dry run for transferring a repository history into another registered repository as an engineering object rather than a file archive: source and target identity, exact source head, default branch, every branch and tag that must arrive, required permissions, protected-branch limitations, what travels as Git content, what provider state cannot travel, the verification procedure, and a SECRET_CONFIG_HANDOVER checklist carrying reference NAMES only. Never uses a ZIP, and never makes the target canonical.',inputSchema:{projectId,sourceResourceId:entityId,targetResourceId:entityId},annotations:ro},safe(async({projectId,sourceResourceId,targetResourceId})=>admin().repositoryExportPlan(principal,projectId,sourceResourceId,targetResourceId)));
  server.registerTool('superadmin_repository_export',{description:'Dispatch the bounded Git-level mirror transfer between two registered repositories, pinned to the exact source head the plan reported. Refuses a non-empty target rather than force-pushing over it. Produces a REPOSITORY_EXPORT_REPORT and a SECRET_CONFIG_HANDOVER; no secret, token, password or .env value is ever transferred. Completing an export does NOT make the target canonical: that requires a separate explicit promotion.',inputSchema:{operationId,projectId,sourceResourceId:entityId,targetResourceId:entityId,expectedSourceHeadSha:z.string().regex(/^[0-9a-f]{40}$/),confirmation:z.literal('EXPORT_REPOSITORY_HISTORY'),reason:z.string().min(8).max(500)},annotations:{...mut,openWorldHint:true}},safe(async value=>admin().repositoryExport(principal,value)));
  server.registerTool('superadmin_repository_export_verify',{description:'Read the target repository back and judge the transfer: source and target identity, source and target head, default branch, every required branch ref and tag at the same commit, and whether the source head commit is actually reachable in the target. A transfer whose history cannot be proved present is BLOCKED, never reported as partial success. Records a REPOSITORY_EXPORT_VERIFICATION artifact.',inputSchema:{operationId,projectId,sourceResourceId:entityId,targetResourceId:entityId},annotations:{...mut,idempotentHint:true}},safe(async value=>admin().repositoryExportVerify(principal,value)));

  server.registerTool('superadmin_developer_handover_report',{description:'Machine-checkable readiness of the canonical development repository for a human backend developer who has no MCP client and no Superadmin token: an ACTIVE canonical binding, README and quick start, the docs/handover package, an .env.example free of secret material, migration instructions, runnable test and build commands, documented contracts, an ownership map, an infrastructure and access inventory with explicit per-item status, troubleshooting, and the exact default-branch commit the documentation was read at. It fails the report when the local development instructions require Backend Autopilot tooling at all. Judges objective facts only, never writing quality. Read-only unless operationId is supplied, which persists a DEVELOPER_HANDOVER_REPORT.',inputSchema:{projectId,operationId:operationId.optional()},annotations:ro},safe(async({projectId,operationId})=>admin().developerHandoverReport(principal,projectId,operationId)));
