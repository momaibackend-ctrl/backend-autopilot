...
Deno.serve(async request=>{
  ...
  server.registerTool('superadmin_epic_verify',{description:'Judge a whole epic at ONE named head commit rather than as the union of its members past verdicts. Aggregates contracts, consumers, invariants, integration dependencies, security/privacy, migrations and cross-module journeys into one matrix; every dimension resolves to PASS, NOT_APPLICABLE with a reason, or BLOCKED with a remediation, and member evidence produced at an earlier commit counts as stale rather than as a pass. Read-only unless persist is true.',inputSchema:{projectId,epicKey:z.string().min(1),headSha:z.string().regex(/^[0-9a-f]{40}$/),taskIds:z.array(entityId).optional(),externalKeyPrefix:z.string().min(1).optional(),persist:z.boolean().optional(),operationId:z.string().min(8).optional()},annotations:ro},safe(async value=>{admin();return runtime.service.epicVerification(value,principal.actor);}));
  server.registerTool('superadmin_epic_evidence_record',{description:'Record one epic dimension result from a run that actually happened, bound to the exact commit it ran on. source is mandatory so the verdict is attributable (a workflow run URL or job id). Evidence is never reusable for a later head commit.',inputSchema:{operationId,projectId,epicKey:z.string().min(1),dimension:epicDimensionSchema,commitSha:z.string().regex(/^[0-9a-f]{40}$/),passed:z.boolean(),source:z.string().min(1),detail:z.string().optional()},annotations:mut},safe(async value=>admin().executeMutation(principal,'epic_evidence_record',value.projectId,value.operationId,value,()=>runtime.service.epicEvidenceRecord(value,principal.actor))));
