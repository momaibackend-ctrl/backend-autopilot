...
Deno.serve(async request=>{
  ...
  server.registerTool('superadmin_task_readiness',{description:'Report what a task still needs to reach READY: the next tool to call, every blocker with concrete remediation, and which formal gate artifacts are present or missing. Safe to call at any point, including before any work has run.',inputSchema:{projectId,taskId:entityId},annotations:ro},safe(async({projectId,taskId})=>{admin();return runtime.service.taskReadiness(projectId,taskId);}));
