...
import { GitHubRestRepositoryProvider } from '../../../packages/adapters/github/src/repository-provider.ts';
import { R2ArtifactBlobStore, readR2ConfigFromEnv } from '../../../packages/adapters/r2/src/artifact-storage.ts';
...
export function createEdgeRuntime(){
  ...
  // control-state store (PostgrestStateStore) regardless of which ArtifactBlobStore is selected.
  ...
  const r2Config=readR2ConfigFromEnv(name=>Deno.env.get(name));
  const blobs=r2Config?new R2ArtifactBlobStore(r2Config.accountId,r2Config.bucketName,r2Config.accessKeyId,r2Config.secretAccessKey):new SupabaseStorageArtifactBlobStore(url,serviceKey);
...
export async function authenticatedOperator(request:Request,projectId?:string){
  ...
  if(!operator[0]){const allowed=csv(required('AUTOPILOT_OPERATOR_EMAILS')).map(value=>value.toLowerCase());if(!allowed.includes(user.email.toLowerCase())&&!superadmins.includes(user.email.toLowerCase()))throw new EdgeHttpError(403,'OPERATOR_REQUIRED','User is not an allowlisted Backend Autopilot operator');await adminWrite(url,key,'autopilot_operators',{user_id:user.id,email:user.email,status:'ACTIVE',role:desiredRole,updated_at:new Date().toISOString()});for(const id of csv(required('AUTOPILOT_OPERATOR_PROJECT_IDS')))await adminWrite(url,key,'autopilot_project_memberships',{user_id:user.id,project_id:id,role:'ADMIN',updated_at:new Date().toISOString()});operator=await adminRows(url,key,`autopilot_operators?select=user_id,status,role&user_id=eq.${user.id}&limit=1`);}else if(desiredRole==='SUPERADMIN'&&operator[0].role!=='SUPERADMIN'){await adminPatch(url,key,'autopilot_operators',`user_id=eq.${user.id}`,{role:'SUPERADMIN',updated_at:new Date().toISOString()});operator[0].role='SUPERADMIN';}
  if(operator[0]?.status!=='ACTIVE')throw new EdgeHttpError(403,'OPERATOR_DISABLED','Operator is disabled');if(projectId&&operator[0].role!=='SUPERADMIN'){const membership=await adminRows<{role:string}>(url,key,`autopilot_project_memberships?select=role&user_id=eq.${user.id}&project_id=eq.${projectId}&limit=1`);if(!membership[0])throw new EdgeHttpError(403,'PROJECT_ACCESS_DENIED','Operator is not assigned to this project');}
  return {...user,role:operator[0].role as 'OPERATOR'|'SUPERADMIN'};
}
...
export async function adminPatch(url:string,key:string,table:string,query:string,body:unknown){const response=await fetch(`${url}/rest/v1/${table}?${query}`,{method:'PATCH',headers:{apikey:key,authorization:`Bearer ${key}`,'content-type':'application/json',prefer:'return=minimal'},body:JSON.stringify(body)});if(!response.ok)throw new EdgeHttpError(502,'CONTROL_STORE_ERROR','Control store update failed',{status:response.status});}
...
async function deploymentStatus(){const repository=required('AUTOPILOT_CONTROL_REPOSITORY');if(!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository))throw new Error('Invalid control repository');const response=await fetch(`https://api.github.com/repos/${repository}/actions/runs?per_page=10`,{headers:{authorization:`Bearer ${required('AUTOPILOT_GITHUB_DISPATCH_TOKEN')}`,accept:'application/vnd.github+json','user-agent':'backend-autopilot'}});const body=response.ok?await response.json() as {workflow_runs?:Array<{id:number;name:string;status:string;conclusion:string|null;head_sha:string;html_url:string;created_at:string}>}:{workflow_runs:[]};return {controlPlane:'SUPABASE',execution:'GITHUB_ACTIONS',console:'GITHUB_PAGES',alwaysOnServer:false,productionWrites:'NOT_SUPPORTED',edgeFunctions:['control-api','mcp','reconcile'],actionsRuns:(body.workflow_runs??[]).map(value=>({id:value.id,name:value.name,status:value.status,conclusion:value.conclusion,headSha:value.head_sha,url:value.html_url,createdAt:value.created_at}))};}
