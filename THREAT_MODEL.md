...
| API Runner / scenario runner SSRF / origin escape | Exact project-owned `HTTP_API` allowlist, non-production mode, HTTPS (loopback HTTP only for `LOCAL`/`SANDBOX`), rejection of private/link-local/unique-local/CGNAT/metadata address classes, same-origin and base-path containment, bounded same-origin-only redirects, bounded timeout and response size | Post-resolution DNS rebinding is only mitigated indirectly (IP-literal rejection plus mandatory TLS hostname binding for non-loopback targets); a hardened outbound proxy with pinned resolution remains required before remote multi-tenant deployment |
